...
export const getPermissionRoleEffective = (roleName: RoleItemType) => {
    ...
    const addRole: any = getDefaultRolePermission()[roleName];
    if (addRole.content) baseRole.content = Array.from(new Set([...baseRole.content, ...addRole.content])).join(",")
    if (addRole.component) baseRole.component = Array.from(new Set([...baseRole.component, ...addRole.component])).join(",")
...
export const getPermissionComponent = () => {
    ...
    Object.values(RoleType).forEach((role: any) => {
        permission.push({ role: role, component: getPermissionRole(role).component, title: toTitleCase(role) })
    });
    return permission;
}
...
export const getPermissionContent = () => {
    const permission: PermissionModel = [];
    Object.values(RoleType).forEach((role: any) => {
        permission.push({ role: role, content: getPermissionRole(role).content, title: toTitleCase(role) })
    });
    return permission;
}
